...
import com.example.unl_pos12.model.telegram.UserData;
...
@Controller
public class TelegramSendingResumeAndReport {
    @Autowired
    ...
    public String uploadResume(UserData userData, int vacancyId) {
        ...
        try {
            ...
            if (accessToken == null) {
                result = "Ошибка: не удалось получить токен доступа. Вам необходимо авторизироваться.";
                return result;
            }
            ...
            if (vacancy.getSiteName().contains("robota.ua")) {
                ...
                if (responseLoadAndSend.getStatusCode() == HttpStatus.OK) {
                    ...
                    result = "Ошибка отправки резюме: " + responseLoadAndSend.getStatusCode() + " - " + responseLoadAndSend.getBody() + " \n" +
            ...
            if (response.getStatusCode() == HttpStatus.OK) {
                ...
                result = "Ошибка отправки резюме: " + response.getStatusCode() + " - " + response.getBody() + "\n" +
                        "submitPageUrl: " + submitPageUrl;
                emailRequest.setTo(email);
                emailRequest.setSubject("Отправка резюме");
                emailRequest.setBody("Вынуждены вас огорчить, ваше резюме не доставлено работодателю. Попробуйте снова.");
            ...
            result = "Ошибка отправки резюме: " + e.getMessage() + "\n" +
                    "submitPageUrl: " + submitPageUrl;
            emailRequest.setTo(email);
            emailRequest.setSubject("Отправка резюме");
